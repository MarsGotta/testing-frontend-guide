...
export default function Sidebar({ onNavigate }: SidebarProps) {
  ...
  const fundamentales = SECTIONS.filter((s) => s.group !== 'avanzado')
  const avanzados = SECTIONS.filter((s) => s.group === 'avanzado')

  function renderSection(section: (typeof SECTIONS)[number], idx: number) {
  ...
  return (
    <nav className="flex flex-col h-full py-4 px-2">
      ...
      <ul className="space-y-0.5 flex-1 overflow-y-auto">
        {fundamentales.map((section, idx) => renderSection(section, idx))}
        ...
        {avanzados.map((section, idx) => renderSection(section, fundamentales.length + idx))}
      </ul>
